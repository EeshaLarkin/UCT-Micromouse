...
5. **WARNING: DO NOT initialize the battery connection while the board is powered.** Under normal operation, the battery is plugged into the power board exactly once (during initial assembly) and remains connected. When initially plugging in the battery connector, **ensure the board is completely unpowered (disconnect all USB cables)**. Connecting the battery while the board is powered (e.g., via USB) will cause the onboard charging/boost circuitry to fail catastrophically (with a high risk of the charging chip catching fire).
