...
* **Permanent Prohibition of C Boot Auto-Format:** In standard MicroPython, boot errors trigger `factory_reset_create_filesystem()`, which calls `f_mkfs` and overwrites `main.py`. Because the filesystem is on external SPI flash (`ZD25WQ80C`), `factory_reset_create_filesystem()` and `factory_reset_make_files()` in `board_init.c` are overridden as no-ops. MicroPython is strictly forbidden from wiping or formatting the external flash behind the user's back. Formatting is performed explicitly via `python tools/deploy.py --engine micropython --factory-reset` or `os.VfsFat.mkfs(pyb.Flash())`.
