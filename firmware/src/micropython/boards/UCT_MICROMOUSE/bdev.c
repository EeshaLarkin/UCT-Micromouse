...
static int ext_flash_init(void) {
    ...
    // Explicitly configure SPI2 GPIO Alternate Function pins (PB13=SCK, PB14=MISO, PB15=MOSI)
...
int uct_bdev_writeblocks(const uint8_t *src, uint32_t block_num, uint32_t num_blocks) {
    ...
    // Flush dirty sectors immediately to guarantee physical flash persistence across power loss
    ext_flash_flush();
